...
#' Look at the DiAna website fore extensive tutorials to practically use the function (\code{vignette("Disproportionality Analysis", package = "DiAna")})
#' and to design and run disproportionality analysis based on expected biases (\code{vignette("Causal Inference", package = "DiAna")})
#' Contrary to `disproportionality_comparison()`, it calculates the association based on the Drug and Reac database,
#' and not based on number of reports.
#' See `disproportionality_trend()` for investigating how disproportionality changed through time.
#'
...
#' Check `disproportionality_analysis()` for more options to address biases.
...
#' Visualizes the results of`disproportionality_trend()`.
#'
#' @family visualization functions
