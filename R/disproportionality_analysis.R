  ...
  if(order=="reac"){
    for (n in 1:length(reac_selected)){
      ...
      for (d in temp_drug_selected){
        ...
        results <-
          data.table(drug = paste0(d,collapse = "; "), event = ifelse(event_names=="none",paste0(r,collapse = "; "),event_names[[n]]), ROR = ROR,
  ...
  if(order=="drug"){
    for (n in 1:length(drug_selected)){
      ...
      for (r in 1:length(temp_reac_selected)){
        ...
        results <-
          data.table(drug = paste0(d,collapse = "; "), event = ifelse(event_names=="none",paste0(reac_selected[r],collapse = "; "),event_names[[r]]), ROR = ROR,
                     ROR_lower, ROR_upper, p_value_fisher, IC, IC_lower,IC_upper, cases = D_AE,
                     Drug_n = D, Event_n = AE,TOT)
        ROR_df <- rbindlist(list(ROR_df, results))
      }
